...
class LogisticRegression:
    ...
    def forward(self, weights, features, labels): #, batch_weight):
        ...
        # print("sigmoid_z:", sigmoid_z.shape)
    ...
    def fit_model(self, X_train, Y_train, instances_count):
        ...
        while self.n_iteration < self.max_iter:
            ...
            for batch_idx, batch_data in enumerate(X_batch_list):
                # print(batch_idx, y_batch_list[batch_idx])
                print("ID:    ", batch_idx)
    ...
    def _generate_batch_data(self, X, y, batch_size):
        ...
        # print("DATA: ", X_batch_list[0], y_batch_list[0])
...
if __name__ == "__main__":
    ...
    print(weight_vector)
    # 模型实例化
    LogisticRegressionModel = LogisticRegression(weight_vector = weight_vector, batch_size = 20, max_iter = 200, alpha = 0.0001, eps = 1e-5)
    # 训练
...
#         # error
#         # self.encrypted_error = sigmoid_z - labels.reshape(-1, 1)
#         # print("labels:", labels.reshape(-1,1).shape)
#         # print("self.encrypted_error: ", self.encrypted_error.shape)
#         # if batch_weight:
...
#         # 计算梯度: (error * X) / Batch
#         # encrypt_g = self.encrypted_error.T.dot(features) * (1 / batch_num)
#         print("error, feature shape: ", error.shape, features.shape)
#         encrypt_g = np.dot(error.T, features) / batch_num
#         # print("encrypt_g: ", encrypt_g.shape)
#         return encrypt_g
...
#         # instances_count = len(Y_train)

#         X_batch_list, y_batch_list, max_epoch = self._generate_batch_data(X_train, Y_train, self.batch_size)
...
#                 if test >= 4: break
#                 print("ID:    ", i)
#                 batch_labels = y_batch_list[i].reshape(-1, 1)
#                 batch_data = X_batch_list[i]
#                 # batch_labels[batch_labels == 0] = 0.01
#                 # batch_labels[batch_labels == 1] = 0.99
...
#                 gradient = self.backward(error = error, features = batch_data, batch_idx = i)
#                 print("gradient shape: ", gradient.shape)
#                 print("gradient: ", gradient)

#                 ## compute loss
#                 batch_loss = self._compute_loss_cross_entropy(weights = self.model_weights, label = batch_labels, batch_idx = i)
#                 # batch_loss = self._compute_loss(y, batch_labels, batch_idx)
...
#         print("batch_num: ", self.batch_num)
#         print(len(X_batch_list), y_batch_list)
#         return X_batch_list, y_batch_list, len(X_batch_list)
