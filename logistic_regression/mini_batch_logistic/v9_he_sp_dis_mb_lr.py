'''
# 目标: 修改成适合本机运行的精简版
# 可以运行SSHE或非SSHE的版本(等v9_3修改完, 再重新复制粘贴这个)
...
PATH_DATA = '/Users/zbz/code/vscodemac_python/hetero_sshe_logistic_regression/data/'
...
class LogisticRegression:
    ...
    def secure_distributed_cal_z(self, X, w1, w2, party = None):
        ...
        if party == "A":
            ...
            # tmp_sum = self.za2_1 + self.za2_2
            # plain_z2 = np.dot(X, w2.T)
            # print("secret za2: ", tmp_sum)
            # print("plain za2: ", plain_z2)
            # if (plain_z2 == tmp_sum).all(): print("ok.")
            # print("secret za1: ", self.za1)

            # print("self.za1 tmp_sum shape: ", self.za1.shape, tmp_sum.shape)
            # za = self.za1.T + tmp_sum
            # print("za: ", za)
            # print("za type, weightA type: ", type(za), type(self.weightA))
            # print("za type, weightA shape: ", za.shape, self.weightA.shape)
            # plain_za = np.dot(X, self.weightA.T)
            # if ( plain_za == za).all(): print("okk.")
            # print("za_plain: ", np.dot(X, self.weightA.T))
    ...
    def fit_model_secure_distributed_input(self, X_trainA, X_trainB, Y_train, instances_count, indice_littleside):
        ...
        while self.n_iteration < self.max_iter:
            ...
            # loss_print = self.cipher.recursive_decrypt(loss)
            # file.write("\rIteration {}, batch sum loss: {}".format(self.n_iteration, loss_print))
            # file.close()
            ############################
...
#     def secure_Matrix_Multiplication(self, matrix, vector_weight, stage = None):
#         '''
#         输入:   数据矩阵和向量
#         返回值: share的两个分片
#         向量加密后做矩阵乘法, 然后 secret share 乘积结果的矩阵分成2个sharings
...
#         if stage == "forward":
#             # print("vector:", vector_weight)
#             # print("vector type: ", type(vector_weight))
#             # print("vector[0] type: ", type(vector_weight[0]))
#             # print("vector shape: ", vector_weight.shape)
#             # sys.exit(0)
#             encrypt_vec = self.cipher.recursive_encrypt(vector_weight)
#             mul_result = np.dot(matrix, encrypt_vec.T)
...
#         wa1, wa2 = self.secret_share_vector_plaintext(self.weightA)
#         wb1, wb2 = self.secret_share_vector_plaintext(self.weightB)
#         # print("wa1 type: ", type(wa1))
#         # print("wa1 shape: ", wa1.shape)
#         # print("wb1 type: ", type(wb1))
...
#                 self.za = self.za1 + self.za2_1 + self.zb1_1
#                 self.zb = self.zb2 + self.za2_2 + self.zb1_2
#                 # print(type(self.za))
#                 # print(type(self.zb))
#                 encrypt_zb = self.cipher.recursive_encrypt(self.zb)
...
#                 self.encrypted_sigmoid_wx = self._compute_sigmoid(self.encrypt_wx)
#                 # error
#                 self.encrypted_error = self.encrypted_sigmoid_wx - batch_labels
#                 yb_s, ya_s = self.secret_share_vector_plaintext(self.encrypted_sigmoid_wx)
#                 error_b = yb_s - batch_labels
#                 error_a = ya_s
#                 ########
